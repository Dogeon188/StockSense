...
class Portfolio:
    def __init__(self, percentages_of_stocks_and_cash, total_money, prices_of_stocks):
        ...
        for idx, percentage in enumerate(percentages_of_stocks_and_cash[:-1]):
            self.amount_of_stocks.append(percentage * total_money / prices_of_stocks[idx])
    ...
    def trade_to_new_percentages(self, percentages_of_stocks_and_cash, prices_of_stocks, trading_fees):   
        ...
        for idx, (percentage, price) in enumerate(zip(percentages_of_stocks_and_cash[:-1], prices_of_stocks)):
            stock_trade = (percentage * total_money / price) - self.amount_of_stocks[idx]
            if stock_trade > 0:
                stock_trade = stock_trade / (1 - trading_fees + trading_fees * percentage)
                cash_trade = - stock_trade * price
                self.amount_of_stocks[idx] = self.amount_of_stocks[idx] + stock_trade * (1 - trading_fees)
...
class MultiStockTradingEnv(_gym.Env):
    ...
    def _get_obs(self):
        ...
        if self.windows is None:
            ...
            _step_index = _np.arange(self._idx + 1 - self.windows , self._idx + 1)
    ...
    def reset(self, seed = None, options=None):
        ...
        self._step = 0
        self._percentages_of_stocks_and_cash = [0] * self.number_of_stocks + [1] # 1 for cash
    ...
    def step(self, actions = None):
        if actions is not None: self._take_action(actions)
        self._idx += 1
        self._step += 1
    ...
    def log(self):
        if self.verbose > 0:
            print("Portfolio Return : " + f"{100*(self.historical_info.get_total_assets(-1) / self.historical_info.get_total_assets(0) -1):5.2f}%")
