...
export async function preprocessData(url: string, type: string){
    ...
    for(let i=data.length-1; i>=0; i--){
        let pair = type === "area" ? { time: data[i].date.split(' ')[0], 
                                       value: Number(data[i].close) }
...
// show AreaChart
export function showAreaChart(state: { chart: string }, areaSeries: ISeriesApi<'Area'>, candlestickSeries: ISeriesApi<'Candlestick'>) {
    state.chart = "area";
...
// show KLineChart
export function showCandlestickChart(state: { chart: string }, areaSeries: ISeriesApi<'Area'>, candlestickSeries: ISeriesApi<'Candlestick'>) {
    state.chart = "kLine";
    areaSeries.applyOptions({ visible: false });
    candlestickSeries.applyOptions({ visible: true });
}

export function createToolTip(state: { chart: string }, areaSeries: ISeriesApi<'Area'>, candlestickSeries: ISeriesApi<'Candlestick'>, chart: IChartApi) {
    ...
    chart.subscribeCrosshairMove(param => {
        if (
            ...
            const data = state.chart === "area" ? param.seriesData.get(areaSeries) : param.seriesData.get(candlestickSeries);
            ...
            if (left > container.clientWidth + container.getClientRects()[0].x - toolTipWidth) {
                left = param.point.x + container.getClientRects()[0].x - toolTipMargin - toolTipWidth;
            }
