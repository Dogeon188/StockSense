...
Despite these advancements in stock price prediction, they are usually evaluated on historical data and do not consider the trading strategy's performance in real-world scenarios. Such problem has been addressed by Feng et al. \cite{feng2019rank} who proposed an improved ranking model that can generate trading signals based on the predicted stock returns. Importantly, they considered the relationship between different stocks that conveys valuable information for trading decisions.

Deep reinforcement learning (DRL) is capable of learning complex trading strategies directly from market data, making it an attractive approach for algorithmic trading. For example, Deep Q-Learning (DQN) has been widely applied to stock trading for decision-making in dynamic and uncertain environments.

% Gu et al. \cite{gu2018} proposed a deep Q-learning framework for stock trading, which outperformed traditional methods in terms of portfolio returns and Sharpe ratios. Jiang et al. \cite{jiang2017} developed a deep reinforcement learning model that can adapt to changing market conditions and showed competitive performance compared to traditional methods.
